...
FindReplaceDialog::FindReplaceDialog(QPlainTextEdit *editor, QWidget *parent):QDialog(parent),m_editor(editor){
    setWindowTitle(tr("查找和替换"));
    setupUi();
    setModal(false);
}

void FindReplaceDialog::setEditor(QPlainTextEdit *editor){ m_editor = editor;}
...
void FindReplaceDialog::onFindNext(){ doFind(true); }

void FindReplaceDialog::onFindPrevious(){ doFind(false); }
...
bool FindReplaceDialog::doFind(bool forward){
    ...
    if (m_regexCheckBox->isChecked()) {
        QRegularExpression regex(searchText);//正则表达式
        if (!m_caseSensitiveCheckBox->isChecked()) {
...
void FindReplaceDialog::onReplaceAll()
{
    ...
    QMessageBox::information(this,tr("替换"),tr("共替换了 %1 处").arg(count));
}